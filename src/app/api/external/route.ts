...
export async function POST(req: NextRequest) {
  ...
  await prisma.$transaction(async (tx) => {
    if (action === "deduct") {
      ...
      // Grant: deduct from API key owner's wallet
      const ownerWallet = await tx.wallet.findUnique({ where: { userId: apiKey.userId } });
      if (!ownerWallet || ownerWallet.balance < amount) throw new Error("APIキー所有者の残高不足");
      await tx.wallet.update({ where: { userId: apiKey.userId }, data: { balance: { decrement: amount } } });
