      ...
      <div className="mb-6 rounded border border-[var(--border)] bg-[var(--card)] p-4 text-xs text-[var(--text-dim)] space-y-3">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 sm:grid-cols-3">
            {[
              ["名前変更", "+8%"],["ユーザー名(@)変更", "+12%"],["アイコン変更", "+5%"],
              ["バナー変更", "+3%"],["bio変更", "+2%"],["bio内リンク変更", "+3%"],
              ["プロフィールURL変更", "+2%"],["場所変更", "+1%"],["居住国変更", "+1%"],
              ["bioにメンション追加", "+2%"],
              ["フォロワー+1%以上", "+30%"],["フォロワー-1%以上", "-30%"],
              ["フォロー数急増(+5%)", "+5%"],["大量フォロー(スパム疑い)", "-5%"],["フォロー整理", "+3%"],
              ["認証取得", "+20%"],["認証剥奪", "-15%"],
              ["凍結", "-50%"],["復活", "+30%"],
              ["鍵垢", "-10%"],["鍵解除", "+5%"],
              ["総いいね節目突破", "+3%"],["総ツイート節目突破", "+5%"],
              ["メディア投稿節目突破", "+2%"],["アカウント周年", "+10%"],
              ["認証ユーザーから言及", "+高"],["ツイート活動(いいね/RT/引用)", "±変動"],
            ].map(([k,v]) => (
              <span key={k} className={`${v.startsWith("+") ? "text-green-400" : v.startsWith("-") ? "text-red-400" : "text-yellow-400"}`}>{v} {k}</span>
            ))}
          </div>
        </div>
        <div>
